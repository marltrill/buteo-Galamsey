...
from buteo.machine_learning.model_base import create_model
from buteo.machine_learning.utils import tpe, SaveBestModel, TimingCallback, OverfitProtection

from tensorflow.python.keras.callbacks import EarlyStopping
from tensorflow.python.keras.models import load_model

#Use eGPU for better processing times and avoid memory limitations
os.environ["TF_GPU_ALLOCATOR"] = "cuda_malloc_async"
os.environ["TF_FORCE_GPU_ALLOW_GROWTH"] = "true"
os.environ["CUDA_VISIBLE_DEVICES"] = "0"
tf.keras.mixed_precision.set_global_policy('mixed_float16')
# %%
folder = "D:/MALT/ghana/data/"

#the rgbn array contains vegetation indices as well
#Load test data
data_test = np.load(folder + "ghana_mines_test.npz")
...
#Load train data
data_train = np.load(folder + "ghana_mines_train.npz")
...
monitor = "val_loss"
outdir = "D:/MALT/ghana/models/VI/3/"
model_name = "ghana_mines_v3_VI03" 
donor_model = None
...
for phase in range(len(fits)):
    ...
    model.fit(
        ...
        callbacks=[
            save_best_model,
            EarlyStopping(          #It gives the model 5 epochs to improve results based on val_loss value, if it doesnt improve-stops decreasing, the model training i stopped. If this continues, it would be overfitting 
                monitor=monitor,    
                patience=5 if phase != 0 else 10,
                min_delta=min_delta,
                mode="min",         #Loss function is suppose to minimize
                baseline=best_val_loss,     #Fit has to improve upon a baseline
